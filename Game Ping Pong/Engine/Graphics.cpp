...
void Graphics::DrawSolidCircle(int a, int b, int r, Color c)
{
	assert(a - r >= 0);
	assert(a + r < Graphics::ScreenWidth);
	assert(b - r >= 0);
	assert(b + r < Graphics::ScreenHeight);
...
void Graphics::DrawOutlineCircle(int a, int b, int r, Color c)
{
	assert( a - r >= 0);
	assert( a + r < Graphics::ScreenWidth);
	assert( b - r >= 0);
	...
	const int halfSqrtR = int(0.71f * r);

	// Draw up and bottom parts
	for (int x = a - halfSqrtR; x < a + halfSqrtR; x++)
	{
		int tmp = (int)sqrt(r * r - (x - a) * (x - a));
		PutPixel(x, b - tmp, c);
		PutPixel(x, b + tmp, c);
	}

	// Draw left and right parts
	for (int y = b - halfSqrtR; y < b + halfSqrtR; y++)
	{
		int tmp = (int)sqrt(r * r - (y - b) * (y - b));
		PutPixel(a - tmp, y, c);
		PutPixel(a + tmp, y, c);
	}
}
...
{
	DrawRect(x0, y0, x1, y1, Color(r, g, b));
}

void Graphics::DrawRectDim(int x, int y, int w, int h, Color c)
{
	DrawRect(x, y, x + w, y + h, c);
}

void Graphics::DrawRectDim(int x, int y, int w, int h, int r, int g, int b)
{
	DrawRectDim(x, y, w, h, Color(r, g, b));
}

void Graphics::DrawDigit(int digit, int x, int y, int r, int g, int b)
{
	assert(digit >= 0 && digit <= 9);

	switch (digit)
	{
		case 0: ch0(x, y, r, g, b); return;
		case 1: ch1(x, y, r, g, b); return;
		case 2: ch2(x, y, r, g, b); return;
		case 3: ch3(x, y, r, g, b); return;
		case 4: ch4(x, y, r, g, b); return;
		case 5: ch5(x, y, r, g, b); return;
		case 6: ch6(x, y, r, g, b); return;
		case 7: ch7(x, y, r, g, b); return;
		case 8: ch8(x, y, r, g, b); return;
		case 9: ch9(x, y, r, g, b); return;
	}
}


void Graphics::Block(int x, int y, int r, int g, int b)
{
	DrawRectDim(x, y, 4, 4, r, g, b);
}
